...
export default function ContentOperationsPage(): ReactElement {
  ...
  return (
    <main className="mx-auto grid min-h-svh w-[calc(100%-32px)] max-w-[1180px] gap-10 py-10 sm:w-[calc(100%-48px)] sm:py-16">
      <header className="grid gap-5 border-b border-cove-default pb-8">
        <Link className={`w-fit text-cove-body-sm text-cove-link underline ${focusRing}`} href="/">
          Back to home
        </Link>
      ...
      <section className="grid gap-4" aria-labelledby="candidate-title">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <h2 id="candidate-title" className="mt-2 font-cove-display text-cove-h2 font-semibold">
              {model.title}
            </h2>
          </div>
      ...
      <aside
        ...
        role="note"
      >
        This local preview contains one original fixture and one reviewed source link. It stores no
        third-party statement, solution, test, or credential.
      </aside>
    </main>
  );
}
